...
log_step "Importing Gentoo binary package GPG keys..."
getuto || die "getuto failed — cannot verify binary packages"
log_info "Binary package keys imported"
...
# Break libwebp <-> tiff circular dependency before full @world update
log_step "Pre-installing tiff+libwebp to break circular dep (--nodeps)..."
emerge --oneshot --nodeps media-libs/tiff media-libs/libwebp \
    || die "Failed to pre-install tiff/libwebp"
